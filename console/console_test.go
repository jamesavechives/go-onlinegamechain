...
import (
	...
	"github.com/onlinegamechain/go-onlinegamechain/internal/jsre"
...
type tester struct {
	workspace string
	stack     *node.Node
	onlinegamechain  *eth.Onlinegamechain
	console   *Console
...
func newTester(t *testing.T, confOverride func(*eth.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Onlinegamechain service within
	...
	ethConf := &eth.Config{
		...
		OGCbase: common.HexToAddress(testAddress),
	...
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return eth.New(ctx, ethConf) }); err != nil {
		t.Fatalf("failed to register Onlinegamechain protocol: %v", err)
	}
	...
	stack.Service(&onlinegamechain)

	return &tester{
		workspace: workspace,
		stack:     stack,
		onlinegamechain:  onlinegamechain,
		console:   console,
		input:     prompter,
		output:    printer,
	}
