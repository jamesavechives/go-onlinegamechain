...
const (
	...
	URN_WANOGCnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANOGCnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: OGCnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, OGCnet
//
...
// WANOGCnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANOGCnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANOGCnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANOGCnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANOGCnetLinkConfig1Clients() (clients []*WANOGCnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANOGCnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANOGCnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANOGCnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANOGCnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANOGCnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANOGCnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOGCnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANOGCnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANOGCnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANOGCnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANOGCnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANOGCnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANOGCnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANOGCnetLinkConfig1 {
	clients := make([]*WANOGCnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANOGCnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewOGCnetLinkStatus: allowed values: Up, Down
func (client *WANOGCnetLinkConfig1) GetOGCnetLinkStatus() (NewOGCnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewOGCnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANOGCnetLinkConfig_1, "GetOGCnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewOGCnetLinkStatus, err = soap.UnmarshalString(response.NewOGCnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
