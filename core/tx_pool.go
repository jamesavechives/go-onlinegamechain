...
type TxPoolConfig struct {
	NoLocals  bool          // Whogc local transaction handling should be disabled
...
// validateTx checks whogc a transaction is valid according to the consensus
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.Gas() < intrGas {
		return errors.New(strconv.Itoa(int(tx.Gas()))+","+strconv.Itoa(int(intrGas)))
