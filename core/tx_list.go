...
// Remove deletes a transaction from the maintained map, returning whogc the
...
type txList struct {
	strict bool         // Whogc nonces are strictly continuous or not
...
// Overlaps returns whogc the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whogc the
...
// Remove deletes a transaction from the maintained list, returning whogc the
...
// Empty returns whogc the list of transactions is empty or not.
...
// Underpriced checks whogc a transaction is cheaper than (or as cheap as) the
