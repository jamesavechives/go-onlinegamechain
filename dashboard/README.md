## Go Onlinegamechain Dashboard

The dashboard is a data visualizer integrated into geth, intended to collect and visualize useful information of an Onlinegamechain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-onlinegamechain` repo), these need to be installed first:
...
For more IDE support install the `linter-eslint` package too, which finds the `.eslintrc` file, and provides real-time linting. Atom warns, that these two packages are incompatible, but they seem to work well togogc. For third-party library errors and auto-completion [flow-typed][flow-typed] is used.
