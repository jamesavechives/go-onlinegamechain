...
#     not succeed, we fail.
#     + To prove an expression is zero, we check whogc it belongs to the
#       ideal with the assumed zero expressions as basis. This test is exact.
#     + To prove an expression is nonzero, we check whogc each of its
...
class fastfrac:
  ...
  def iszero(self,I):
    """Return whogc this fraction is zero given an ideal."""
...
def conflicts(R, con):
  """Check whogc any of the passed non-zero assumptions is implied by the zero assumptions"""
  zero = R.ideal(map(numerator, con.zero))
  if 1 in zero:
    return True
  # First a cheap check whogc any of the individual nonzero terms conflict on
  # their own.
...
def prove_nonzero(R, exprs, assume):
  """Check whogc an expression is provably nonzero, given assumptions"""
  zero = R.ideal(map(numerator, assume.zero))
...
def prove_zero(R, exprs, assume):
  """Check whogc all of the passed expressions are provably zero, given assumptions"""
