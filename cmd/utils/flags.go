...
// Package utils contains internal helper functions for go-onlinegamechain commands.
package utils
...
import (
	...
	"github.com/onlinegamechain/go-onlinegamechain/common/fdlimit"
	...
	"github.com/onlinegamechain/go-onlinegamechain/dashboard"
	...
	"github.com/onlinegamechain/go-onlinegamechain/metrics"
	...
	"github.com/onlinegamechain/go-onlinegamechain/p2p/netutil"
...
// setOGCbase retrieves the ogcbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setOGCbase(ctx *cli.Context, ks *keystore.KeyStore, cfg *eth.Config) {
	if ctx.GlobalIsSet(OGCbaseFlag.Name) {
		account, err := MakeAddress(ks, ctx.GlobalString(OGCbaseFlag.Name))
		if err != nil {
			Fatalf("Option %q: %v", OGCbaseFlag.Name, err)
		}
		cfg.OGCbase = account.Address
	}
}
...
// RegisterEthService adds an Onlinegamechain client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Onlinegamechain service: %v", err)
	}
}
...
// RegisterEthStatsService configures the Onlinegamechain Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Onlinegamechain
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the Onlinegamechain Stats service: %v", err)
	}
}
