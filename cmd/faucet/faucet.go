...
// faucet is a OGC faucet backed by a light client.
package main
...
import (
	...
	"github.com/onlinegamechain/go-onlinegamechain/accounts/keystore"
	...
	"github.com/onlinegamechain/go-onlinegamechain/p2p/discover"
	"github.com/onlinegamechain/go-onlinegamechain/p2p/discv5"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Onlinegamechain protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of OGCs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Onlinegamechain and the faucet")
)

var (
	ogc = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s OGCs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Onlinegamechain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Onlinegamechain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Onlinegamechain protocol stack
	client *ethclient.Client   // Client connection to the Onlinegamechain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Onlinegamechain light client protocol
	...
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightOnlinegamechain
			ctx.Service(&serv)
...
// close terminates the Onlinegamechain connection and tears down the faucet.
...
// apiHandler handles requests for OGC grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Onlinegamechain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/onlinegamechain/go-onlinegamechain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), ogc)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, ogc)
...
// the username, avatar URL and Onlinegamechain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Onlinegamechain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Onlinegamechain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Onlinegamechain address and profile URL.
...
// returning the username, avatar URL and Onlinegamechain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Onlinegamechain address,
