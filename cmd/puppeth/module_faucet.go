...
FROM onlinegamechain/client-go:alltools-latest
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Onlinegamechain listener port":       strconv.Itoa(info.node.portFull),
		"Funding amount (base tier)":   fmt.Sprintf("%d OGCs", info.amount),
...
// checkFaucet does a health-check against an faucet server to verify whogc
