...
// nodeDockerfile is the Dockerfile required to run an Onlinegamechain node.
var nodeDockerfile = `
FROM onlinegamechain/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.onlinegamechain/keystore/ && cp /signer.json /root/.onlinegamechain/keystore/' >> geth.sh && \{{end}}
	echo $'geth --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .OGCbase}}--ogcbase {{.OGCbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> geth.sh

ENTRYPOINT ["/bin/sh", "geth.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Onlinegamechain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.onlinegamechain{{if .Ethashdir}}
...
// deployNode deploys a new Onlinegamechain node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.ogcbase == "" {
		kind = "bootnode"
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.ogcbase != "" {
			...
			report["Miner account"] = info.ogcbase
		}
		if info.keyJSON != "" {
...
// whogc it's running, and if yes, whogc it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.onlinegamechain"],
		...
		ogcbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
