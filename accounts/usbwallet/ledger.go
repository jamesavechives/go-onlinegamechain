...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Onlinegamechain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Onlinegamechain transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whogc the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Onlinegamechain app in browser mode", w.failure
	}
	if w.offline() {
		return "Onlinegamechain app offline", w.failure
	}
	return fmt.Sprintf("Onlinegamechain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whogc the wallet and the Onlinegamechain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Onlinegamechain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Onlinegamechain app's version, will fail prior to v1.0.2
...
// and returning the Onlinegamechain address located on that derivation path.
...
//
// Note, if the version of the Onlinegamechain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Onlinegamechain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Onlinegamechain wallet app running
...
// ledgerDerive retrieves the currently active Onlinegamechain address from a Ledger
...
//   Onlinegamechain address length | 1 byte
//   Onlinegamechain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Onlinegamechain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Onlinegamechain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whogc legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the Onlinegamechain signature and do a sanity validation
	if len(reply) != 65 {
