...
import (
	...
	"github.com/onlinegamechain/go-onlinegamechain/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whogc the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whogc the Trezor is opened, closed
// or whogc the Onlinegamechain app was not started on it.
...
// Onlinegamechain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.OnlinegamechainAddress)
	if _, err := w.trezorExchange(&trezor.OnlinegamechainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.OnlinegamechainSignTx{
	...
	response := new(trezor.OnlinegamechainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.OnlinegamechainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
