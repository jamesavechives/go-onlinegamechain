...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Onlinegamechain
...
type wallet struct {
	...
	deriveChain    onlinegamechain.ChainStateReader // Blockchain state reader to discover used account with
...
// whogc they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Onlinegamechain account
...
// Contains implements accounts.Wallet, returning whogc a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain onlinegamechain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
