...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Onlinegamechain protocol")
	...
	log.Info("Light Onlinegamechain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	p.Log().Debug("Light Onlinegamechain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Onlinegamechain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Onlinegamechain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Onlinegamechain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Onlinegamechain message arrived", "code", msg.Code, "bytes", msg.Size)
