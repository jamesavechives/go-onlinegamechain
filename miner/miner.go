...
// Package miner implements Onlinegamechain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whogc we can start the mining operation
	shouldStart int32 // should start indicates whogc we should start after sync
}
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setOGCbase(coinbase)
...
func (self *Miner) SetOGCbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setOGCbase(addr)
}
