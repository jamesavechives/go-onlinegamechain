...
func (self *worker) setOGCbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) commitNewWork() {
	...
	// If we are care about TheDAO hard-fork check whogc to override the extra-data or not
	if daoBlock := self.config.DAOForkBlock; daoBlock != nil {
		// Check whogc the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whogc we support or oppose the fork, override differently
...
func (env *Work) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, coinbase common.Address) {
	...
	for {
		...
		// Check whogc the tx is replay protected. If we're not in the EIP155 hf
