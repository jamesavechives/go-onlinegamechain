...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.onlinegamechain", new NodeConfig());
			node.start();
			...
			OnlinegamechainClient ec = node.getOnlinegamechainClient();
	...
	// signatures too. Regression test for go-onlinegamechain issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogogc
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/onlinegamechain/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.onlinegamechain", "github.com/onlinegamechain/go-onlinegamechain/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "onlinegamechain", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.onlinegamechain.gethtest"
